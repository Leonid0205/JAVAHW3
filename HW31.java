    ...
    class Main
    {
        ...
        private static boolean isValid(int[][] mat, boolean[][] visited, int row, int col)
        {
            return (row >= 0) && (row < mat.length) && (col >= 0) && (col < mat[0].length)
                    && mat[row][col] == 77 && !visited[row][col];
        }
        ...
        public static void main(String[] args)
        {
            ...
            System.out.println();
            int min_dist = findShortestPathLength(mat, 0, 0, 9, 9);
    
            if (min_dist != -1) {
                System.out.println("The shortest path length from start to exit: " + min_dist);
            } else {
                System.out.println("Cannot be reached from source");
            }
            System.out.println();    
            for (int k = 0; k < mat.length; k++) {
                for (int l = 0; l < mat[0].length; l++) {
